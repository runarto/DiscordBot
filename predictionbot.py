...
@bot.slash_command(
    name='ukens_kupong',
    description='Send ukens kupong for de neste dagene',
    guild_ids=[guild_id]
)
async def send_ukens_kupong(ctx, days: int):
...
@bot.slash_command(
    guild_ids=[guild_id],
    description="Vis resultatene fra forrige uke, og totale resultater"
)
async def total_leaderboard(ctx):
    # Sort user scores by points (descending order)
    scores = file_functions.user_scores
...
async def send_leaderboard_message(ctx, days: int):
    try:
        message = format_leaderboard_message(days)
...
@tree.command(name='ukens_kupong', description='Send ukens kupong for de neste dagene')
...
@tree.command(name = "total_ledertavle",  description="Vis totale resultater")
async def total_leaderboard(ctx):
    await tree.sync(guild=discord.Object(id=guild_id))
    # Sort user scores by points (descending order)
    scores = file_functions.user_scores
...
@tree.command(name = "ukens_resultater",description="Vis resultatene fra forrige uke, og totale resultater")
...
@tree.command(name="devlocalsync", description="Synkroniser commands lokalt i serveren")
@commands.has_permissions(administrator=True)  # Restrict this command to administrators
...
"""
tree.add_command(send_ukens_kupong)
tree.add_command(total_leaderboard)
tree.add_command(send_leaderboard_message)
tree.add_command(localsync)
"""